"""
E0509 IK 환경 V4 (Hybrid RL + Classical Control)

=== V4 변경사항 ===
1. 조건 완화 (APPROACH → ALIGN, ALIGN → DESCEND)
2. 초기 자세 높이기 (더 높은 위치에서 시작)
3. Fine Align Mode: DESCEND 조건 달성 시 TCP 제어로 정밀 정렬
4. 정렬 완료 후 수직 하강 + GRASP
...
FINE_ALIGN_ROTATION_SPEED = 0.02  # rad/step
DESCEND_SPEED = 0.003             # m/step
...
# =============================================================================
@configclass
class E0509IKEnvV4Cfg(DirectRLEnvCfg):
    """E0509 IK 환경 V4 설정 (Hybrid RL + TCP)"""
...
class E0509IKEnvV4(DirectRLEnv):
    """E0509 IK 환경 V4 (Hybrid RL + TCP Control)"""

    cfg: E0509IKEnvV4Cfg

    def __init__(self, cfg: E0509IKEnvV4Cfg, render_mode: str | None = None, **kwargs):
    ...
    def _compute_tcp_actions(self, ee_pos: torch.Tensor, ee_quat: torch.Tensor) -> torch.Tensor:
        ...
        # ============================================================
        # DESCEND: 수직 하강 (펜 축 방향으로)
        # ============================================================
        descend_mask = (self.phase == PHASE_DESCEND)
        if descend_mask.any():
            # 펜 축 방향으로 하강 (캡을 향해)
            descend_dir = -pen_z[descend_mask]  # 캡 방향 (pen_z의 반대)
    ...
    def _get_observations(self) -> dict:
        ...
        # 현재 단계 (정규화)
        phase_normalized = self.phase.float() / 4.0  # V4: 5단계

        # TCP 제어 활성화 여부
        tcp_active = ((self.phase >= PHASE_FINE_ALIGN) & (self.phase <= PHASE_GRASP)).float()
    ...
    def get_phase_stats(self) -> dict:
        """단계별 통계"""
        current_phases = torch.bincount(self.phase, minlength=5)
        return {
...
@configclass
class E0509IKEnvV4Cfg_PLAY(E0509IKEnvV4Cfg):
    """테스트용 설정"""
