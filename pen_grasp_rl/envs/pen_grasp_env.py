...
def distance_ee_cap_reward(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    Uses exponential decay for sharper gradient near the target.
    """
    robot: Articulation = env.scene["robot"]
    pen: RigidObject = env.scene["pen"]
    ...
    # Exponential reward: 0m→1.0, 0.1m→0.37, 0.2m→0.14, 0.3m→0.05
    return torch.exp(-distance * 10.0)


def action_rate_penalty(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Penalty for large actions (L2 norm squared).

    Encourages smooth, small actions. Weight controls penalty strength.
    """
...
def z_axis_alignment_reward(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    1. Gripper is close to pen cap (within 10cm)
    ...
    # 2. Only apply when close to cap (within 10cm)
...
class RewardsCfg:
    ...
    action_rate = RewTerm(func=action_rate_penalty, weight=0.01)  # 0.1→0.01 (함수에서 0.001 제거됨)
