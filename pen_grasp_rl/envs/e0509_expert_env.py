"""
E0509 전문가 궤적 환경 (APPROACH Only - Sim2Real Ready)
...
class E0509ExpertEnvCfg(DirectRLEnvCfg):
    """E0509 전문가 궤적 환경 설정 (APPROACH Only - Sim2Real Ready)"""
    ...
    # 시각화 마커 설정
    enable_markers = False  # True로 설정하면 마커 표시
...
class E0509ExpertEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: E0509ExpertEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        print(f"[E0509ExpertEnv] EE body: {body_names[0]} (idx={self._ee_body_idx})")
        print(f"[E0509ExpertEnv] Arm joints: {self._arm_joint_names}")
    ...
    def _setup_scene(self):
        ...
        # 마커 설정 (enable_markers=True일 때만)
        if self.cfg.enable_markers:
            self._setup_markers()

    def _setup_markers(self):
        """시각화 마커 설정 (grasp_point, pen_cap, approach_point)"""
        from isaaclab.markers import VisualizationMarkersCfg

        # Grasp Point 마커 (초록색 구)
        grasp_marker_cfg = VisualizationMarkersCfg(
            prim_path="/Visuals/GraspPoint",
            markers={
                "sphere": SphereCfg(
                    radius=0.015,
                    visual_material=PreviewSurfaceCfg(diffuse_color=(0.0, 1.0, 0.0)),  # 초록색
                ),
            },
        )
        self.grasp_marker = VisualizationMarkers(grasp_marker_cfg)

        # Pen Cap 마커 (빨간색 구)
        cap_marker_cfg = VisualizationMarkersCfg(
            prim_path="/Visuals/PenCap",
            markers={
                "sphere": SphereCfg(
                    radius=0.015,
        ...
        self.cap_marker = VisualizationMarkers(cap_marker_cfg)

        # Approach Point 마커 (파란색 구) - 캡 위 5cm 지점
        approach_marker_cfg = VisualizationMarkersCfg(
            prim_path="/Visuals/ApproachPoint",
            markers={
                "sphere": SphereCfg(
                    radius=0.01,
                    visual_material=PreviewSurfaceCfg(diffuse_color=(0.0, 0.0, 1.0)),  # 파란색
                ),
            },
        )
        self.approach_marker = VisualizationMarkers(approach_marker_cfg)

        self._markers_initialized = True
        print("[E0509ExpertEnv] 시각화 마커 설정 완료")

    def _update_markers(self):
        """마커 위치 업데이트"""
        if not self.cfg.enable_markers or not self._markers_initialized:
            return

        # Grasp Point 위치
        grasp_pos = self._get_grasp_point()
        grasp_quat = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=self.device).repeat(self.num_envs, 1)
        self.grasp_marker.visualize(grasp_pos, grasp_quat)

        # Pen Cap 위치
        cap_pos = self._get_pen_cap_pos()
        cap_quat = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=self.device).repeat(self.num_envs, 1)
        self.cap_marker.visualize(cap_pos, cap_quat)

        # Approach Point (캡 위 5cm) - 펜 축 방향으로 5cm 위
        pen_z = self._get_pen_z_axis()
        approach_pos = cap_pos + pen_z * 0.05  # 캡 위 5cm
        approach_quat = torch.tensor([[1.0, 0.0, 0.0, 0.0]], device=self.device).repeat(self.num_envs, 1)
        self.approach_marker.visualize(approach_pos, approach_quat)
...
class E0509ExpertEnvCfg_PLAY(E0509ExpertEnvCfg):
    """전문가 정책 테스트용 설정 (마커 활성화)"""

    enable_markers = True  # 마커 표시
...
@configclass
class E0509ExpertEnvCfg_L0(E0509ExpertEnvCfg):
    """Level 0: 펜 수직 (기본 동작 학습)"""
    curriculum_level = 0


@configclass
class E0509ExpertEnvCfg_L1(E0509ExpertEnvCfg):
    """Level 1: 펜 10° 기울기"""
    curriculum_level = 1


@configclass
class E0509ExpertEnvCfg_L2(E0509ExpertEnvCfg):
    """Level 2: 펜 20° 기울기"""
    curriculum_level = 2


@configclass
class E0509ExpertEnvCfg_L3(E0509ExpertEnvCfg):
