"""
E0509 IK 환경 V7 학습 스크립트 (APPROACH Only - Sim2Real Ready)

V7 주요 변경사항:
- GRASP 단계 제거 (APPROACH만)
- 성공 조건: 캡 위 위치 + 자세 정렬 + 캡 위에 있어야 함
- 그리퍼 제어 제거 (항상 열린 상태)
- Sim2Real 최적화

사용법:
    # Level 0부터 시작 (펜 수직)
    python train_ik_v7.py --headless --num_envs 4096 --level 0

    # Level 1 (10°) - Level 0 체크포인트에서 이어서
    python train_ik_v7.py --headless --num_envs 4096 --level 1 --checkpoint /path/to/l0_model.pt

    # Level 2 (20°) - Level 1 체크포인트에서 이어서
    python train_ik_v7.py --headless --num_envs 4096 --level 2 --checkpoint /path/to/l1_model.pt

    # Level 3 (30°) - 최종 목표
    python train_ik_v7.py --headless --num_envs 4096 --level 3 --checkpoint /path/to/l2_model.pt

    # Fixed LR 모드 (발산 방지)
    python train_ik_v7.py --headless --num_envs 4096 --level 0 --fixed_lr
...
# 명령행 인자
# =============================================================================
parser = argparse.ArgumentParser(description="E0509 IK V7 환경 학습 (Curriculum Learning)")
...
class E0509IKV5PPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """E0509 IK V7 환경 PPO 설정"""
    seed = 42
    device = "cuda:0"
    num_steps_per_env = 24
    max_iterations = 5000
    save_interval = 100
    experiment_name = "e0509_ik_v7"
...
def get_env_cfg_for_level(level: int):
    ...
    return cfg_map.get(level, E0509IKEnvV7Cfg_L0)()
...
def main():
    ...
    # Runner 생성
    # =============================================================================
    log_dir = f"./pen_grasp_rl/logs/e0509_ik_v7_l{args.level}"
    ...
    print("=" * 70)
    print("E0509 IK V7 환경 강화학습 시작 (Curriculum Learning)")
    print("=" * 70)
