...
```python
# dot < -0.9부터 급격히 증가
exponential_bonus = exp((−dot − 0.9) × 20)
```
...
| 보상 | 스케일 | 설명 |
|------|--------|------|
| pre-grasp 거리 페널티 | -2.0 | 펜캡 위 7cm 위치까지 거리 |
| 진행 보상 | +5.0 | 거리 감소량 |
| 기본 정렬 보상 | +3.0 | (-dot - 0.5) / 0.5 |
| **지수적 정렬 보너스** | +2.0 | exponential_bonus |
| 단계 전환 보너스 | +20.0 | DESCEND 진입 시 |

#### DESCEND 단계
| 보상 | 스케일 | 설명 |
|------|--------|------|
| 펜캡 거리 페널티 | -3.0 | 펜캡까지 거리 |
| 기본 정렬 유지 보상 | +5.0 | -dot |
| **지수적 정렬 보너스** | +4.0 | exponential_bonus × 2 (더 중요) |
| 하강 진행 보상 | +3.0 | 거리 감소량 |
...
| 관절 | 조건 | 페널티 |
|------|------|--------|
| Joint 3 (팔꿈치) | \|pos\| < 0.15 rad | -2.0 |
| Joint 5 (손목) | \|pos\| < 0.15 rad | -2.0 |
